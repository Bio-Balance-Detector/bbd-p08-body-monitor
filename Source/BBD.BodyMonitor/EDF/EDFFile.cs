namespace BBD.BodyMonitor.EDF
{
    ...
    public class EDFFile
    {
        ...
        public static EDFFile FromStream(Stream edfStream)
        {
            ...
            try
            {
                using BinaryReader reader = new(edfStream);
                ...
                for (int i = 0; i < result.NumberOfSignals; i++)
                {
                    Signal signal = new()
                    {
                        Label = new string(reader.ReadChars(16)).Trim(),
                        TransducerType = new string(reader.ReadChars(80)).Trim(),
                        PhysicalDimension = new string(reader.ReadChars(8)).Trim(),
                        PhysicalMinimum = reader.ReadSingle(),
                        PhysicalMaximum = reader.ReadSingle(),
                        DigitalMinimum = reader.ReadSingle(),
                        DigitalMaximum = reader.ReadSingle(),
                        NumberOfSamples = reader.ReadInt32(),
                        Reserved = new string(reader.ReadChars(32)).Trim(),
                        Data = new float[0]
                    };
                ...
                {
                    string annotationData = new(reader.ReadChars(annotationLength));
                    ...
                    foreach (string annotation in annotations)
                    {
                        if (annotation.Length > 0)
                        {
                            ...
                            if (annotationParts.Length > 1)
                            {
                                ...
                                {
                                    TimeStamp = double.Parse(annotationParts[0]),
                                    Description = annotationParts[1]
                                };
        ...
        {
            using BinaryWriter writer = new(edfStream);
